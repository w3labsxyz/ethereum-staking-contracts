...
const JustfarmingCoin = artifacts.require("JustfarmingCoin");

contract("SplitRewards", (accounts) => {
  const [owner, payee1, payee2, payee3, nonpayee1, payer1] = accounts;

  const amount = ether("1");

  it("rejects an empty set of payees", async () => {
    await expectRevert(SplitRewards.new([], []), "SplitRewards: no payees");
  });

  it("rejects more payees than shares", async () => {
    await expectRevert(
      SplitRewards.new([payee1, payee2, payee3], [20, 30]),
      "SplitRewards: payees and shares length mismatch"
    );
  });

  it("rejects more shares than payees", async () => {
    await expectRevert(
      SplitRewards.new([payee1, payee2], [20, 30, 40]),
      "SplitRewards: payees and shares length mismatch"
    );
  });
  ...
  it("rejects null payees", async () => {
    await expectRevert(
      SplitRewards.new([payee1, ZERO_ADDRESS], [20, 30]),
      "SplitRewards: account is the zero address"
    );
  });

  it("rejects zero-valued shares", async () => {
    await expectRevert(
      SplitRewards.new([payee1, payee2], [20, 0]),
      "SplitRewards: shares are 0"
    );
  });

  it("rejects repeated payees", async () => {
    await expectRevert(
      SplitRewards.new([payee1, payee1], [20, 30]),
      "SplitRewards: account already has shares"
    );
  });
  ...
  context("once deployed", () => {
    beforeEach(async function () {
      ...
      this.contract = await SplitRewards.new(this.payees, this.shares);
      this.token = await JustfarmingCoin.new();
    ...
    it("has payees", async function () {
      await Promise.all(
        this.payees.map(async (payee, index) => {
          ...
          expect(await this.contract.released(payee)).to.be.bignumber.equal(
            "0"
          );
          expect(await this.contract.releasable(payee)).to.be.bignumber.equal(
            "0"
          );
        })
      );
    ...
    describe("accepts payments", () => {
      ...
      it("Token", async function () {
        ...
        expect(
          await this.token.balanceOf(this.contract.address)
        ).to.be.bignumber.equal(amount);
      });
    });
    ...
    describe("shares", () => {
      it("stores shares if address is payee", async function () {
        expect(await this.contract.shares(payee1)).to.be.bignumber.not.equal(
          "0"
        );
      });

      it("does not store shares if address is not payee", async function () {
        expect(await this.contract.shares(nonpayee1)).to.be.bignumber.equal(
          "0"
        );
      });
    });
      ...
      describe("Ether", () => {
        it("reverts if no funds to claim", async function () {
          await expectRevert(
            ...
            "SplitRewards: account is not due payment"
          );
        });
        it("reverts if non-payee want to claim", async function () {
          ...
          await expectRevert(
            ...
            "SplitRewards: account has no shares"
          );
        });
      });
    ...
    describe("tracks releasable and released", () => {
      it("Ether", async function () {
        ...
        expect(await this.contract.releasable(payee2)).to.be.bignumber.equal(
          payment
        );
        ...
        expect(await this.contract.released(payee2)).to.be.bignumber.equal(
          payment
        );
      });
      ...
      it("Token", async function () {
        ...
        expect(
          await this.contract.released(this.token.address, payee2)
        ).to.be.bignumber.equal(payment);
      });
    });
    ...
    describe("distributes funds to payees", () => {
      it("Ether", async function () {
        ...
        expect(await this.contract.totalReleased()).to.be.bignumber.equal(
          initBalance
        );
      });
      ...
      it("Token", async function () {
        ...
        expect(await this.token.balanceOf(payee1)).to.be.bignumber.equal(
          ether("0.40")
        );
        expect(await this.token.balanceOf(payee2)).to.be.bignumber.equal(
          ether("0.20")
        );
        expect(await this.token.balanceOf(payee3)).to.be.bignumber.equal(
          ether("1.40")
        );
      });
    });
