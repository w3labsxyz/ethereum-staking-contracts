...
contract StakingVaultTest is Test, StakingVaultSetup {
    ...
    function test_successfulStakingOf32Ether() public {
        ...
        assertEq(pd[0].withdrawalCredentials, vm.parseJsonBytes32(json, ".withdrawal_credentials[0]"));
        assertEq(pd[0].signature, signatures[0]);
        assertEq(pd[0].depositDataRoot, vm.parseJsonBytes32(json, ".deposit_data_roots[0]"));
        ...
        assertFalse(success, "Call succeeded, but should have failed because msg.value < 32 ether");
        ...
        assertFalse(success, "Call succeeded, but should have failed because msg.value was 33 ether");
    ...
    function test_successfulStakingOf64Ether() public {
        ...
        string memory root = vm.projectRoot();
        string memory path = string.concat(root, "/tests/fixtures/64eth-deposit.json");
        string memory json = vm.readFile(path);
        bytes[] memory pubkeys = vm.parseJsonBytesArray(json, ".pubkeys");
        bytes[] memory signatures = vm.parseJsonBytesArray(json, ".signatures");
        uint256[] memory depositValues = vm.parseJsonUintArray(json, ".amounts");
        ...
        StakingVault.DepositData[] memory pd = stakingVaultProxy.depositData(64 ether);
        ...
        assertEq(pd[0].withdrawalCredentials, vm.parseJsonBytes32(json, ".withdrawal_credentials[0]"));
        assertEq(pd[1].withdrawalCredentials, vm.parseJsonBytes32(json, ".withdrawal_credentials[1]"));

        assertEq(pd[0].signature, signatures[0]);
        assertEq(pd[1].signature, signatures[1]);

        assertEq(pd[0].depositDataRoot, vm.parseJsonBytes32(json, ".deposit_data_roots[0]"));
        assertEq(pd[1].depositDataRoot, vm.parseJsonBytes32(json, ".deposit_data_roots[1]"));
    ...
    function test_successfulStakingOf320Ether() public {
        ...
        string memory root = vm.projectRoot();
        string memory path = string.concat(root, "/tests/fixtures/320eth-deposit.json");
        string memory json = vm.readFile(path);
        bytes[] memory pubkeys = vm.parseJsonBytesArray(json, ".pubkeys");
        bytes[] memory signatures = vm.parseJsonBytesArray(json, ".signatures");
        uint256[] memory depositValues = vm.parseJsonUintArray(json, ".amounts");
        ...
        StakingVault.DepositData[] memory pd = stakingVaultProxy.depositData(320 ether);
        ...
        assertEq(pd[0].withdrawalCredentials, vm.parseJsonBytes32(json, ".withdrawal_credentials[0]"));
        assertEq(pd[1].withdrawalCredentials, vm.parseJsonBytes32(json, ".withdrawal_credentials[1]"));
        assertEq(pd[2].withdrawalCredentials, vm.parseJsonBytes32(json, ".withdrawal_credentials[2]"));
        assertEq(pd[3].withdrawalCredentials, vm.parseJsonBytes32(json, ".withdrawal_credentials[3]"));
        assertEq(pd[4].withdrawalCredentials, vm.parseJsonBytes32(json, ".withdrawal_credentials[4]"));
        assertEq(pd[5].withdrawalCredentials, vm.parseJsonBytes32(json, ".withdrawal_credentials[5]"));
        assertEq(pd[6].withdrawalCredentials, vm.parseJsonBytes32(json, ".withdrawal_credentials[6]"));
        assertEq(pd[7].withdrawalCredentials, vm.parseJsonBytes32(json, ".withdrawal_credentials[7]"));
        assertEq(pd[8].withdrawalCredentials, vm.parseJsonBytes32(json, ".withdrawal_credentials[8]"));
        assertEq(pd[9].withdrawalCredentials, vm.parseJsonBytes32(json, ".withdrawal_credentials[9]"));
        ...
        assertEq(pd[0].depositDataRoot, vm.parseJsonBytes32(json, ".deposit_data_roots[0]"));
        assertEq(pd[1].depositDataRoot, vm.parseJsonBytes32(json, ".deposit_data_roots[1]"));
        assertEq(pd[2].depositDataRoot, vm.parseJsonBytes32(json, ".deposit_data_roots[2]"));
        assertEq(pd[3].depositDataRoot, vm.parseJsonBytes32(json, ".deposit_data_roots[3]"));
        assertEq(pd[4].depositDataRoot, vm.parseJsonBytes32(json, ".deposit_data_roots[4]"));
        assertEq(pd[5].depositDataRoot, vm.parseJsonBytes32(json, ".deposit_data_roots[5]"));
        assertEq(pd[6].depositDataRoot, vm.parseJsonBytes32(json, ".deposit_data_roots[6]"));
        assertEq(pd[7].depositDataRoot, vm.parseJsonBytes32(json, ".deposit_data_roots[7]"));
        assertEq(pd[8].depositDataRoot, vm.parseJsonBytes32(json, ".deposit_data_roots[8]"));
        assertEq(pd[9].depositDataRoot, vm.parseJsonBytes32(json, ".deposit_data_roots[9]"));
    ...
    function test_depositorManagementAndStaking() public {
        ...
        // Add depositor1
        vm.prank(staker);
        stakingVaultProxy.grantRole(DEPOSITOR_ROLE, depositor1);
        ...
        // Add depositor2
        vm.prank(staker);
        stakingVaultProxy.grantRole(DEPOSITOR_ROLE, depositor2);
        ...
        // Remove depositor1
        vm.prank(staker);
        stakingVaultProxy.revokeRole(DEPOSITOR_ROLE, depositor1);
    ...
    function test_accessControl() public {
        ...
        vm.prank(staker);
        vm.expectRevert();
        stakingVaultProxy.renounceRole(STAKER_ROLE, staker);

        // The operator can't be removed as operator
        vm.prank(operator);
        vm.expectRevert();
        stakingVaultProxy.renounceRole(OPERATOR_ROLE, operator);

        // The staker can be removed as depositor but also be re-added
        vm.prank(staker);
        stakingVaultProxy.revokeRole(DEPOSITOR_ROLE, staker);
        assertFalse(stakingVaultProxy.hasRole(DEPOSITOR_ROLE, staker));
        vm.prank(staker);
        stakingVaultProxy.grantRole(DEPOSITOR_ROLE, staker);
        assertTrue(stakingVaultProxy.hasRole(DEPOSITOR_ROLE, staker));
    }
    ...
    function test_sequentialStakingWithMultipleQuotas() public {
        ...
        string memory root = vm.projectRoot();
        string memory path32 = string.concat(root, "/tests/fixtures/32eth-deposit.json");
        string memory json32 = vm.readFile(path32);
        bytes[] memory pubkeys32 = vm.parseJsonBytesArray(json32, ".pubkeys");
        bytes[] memory signatures32 = vm.parseJsonBytesArray(json32, ".signatures");
        uint256[] memory depositValues32 = vm.parseJsonUintArray(json32, ".amounts");

        // Operator approves first quota
        vm.prank(operator);
        stakingVaultProxy.approveStakeQuota(pubkeys32, signatures32, depositValues32);
        ...
        assertEq(pd[0].withdrawalCredentials, vm.parseJsonBytes32(json32, ".withdrawal_credentials[0]"));
        assertEq(pd[0].signature, signatures32[0]);
        assertEq(pd[0].depositDataRoot, vm.parseJsonBytes32(json32, ".deposit_data_roots[0]"));
        ...
        string memory path64 = string.concat(root, "/tests/fixtures/64eth-deposit.json");
        string memory json64 = vm.readFile(path64);
        bytes[] memory pubkeys64 = vm.parseJsonBytesArray(json64, ".pubkeys");
        bytes[] memory signatures64 = vm.parseJsonBytesArray(json64, ".signatures");
        uint256[] memory depositValues64 = vm.parseJsonUintArray(json64, ".amounts");

        // Operator approves second quota
        vm.prank(operator);
        stakingVaultProxy.approveStakeQuota(pubkeys64, signatures64, depositValues64);
